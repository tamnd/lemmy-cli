...
describes what users get. Pick a format with `-o`, or let lemmy choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
lemmy <command> -o table   # aligned columns for reading
lemmy <command> -o jsonl   # one JSON object per line, for piping
lemmy <command> -o json    # a single JSON array
lemmy <command> -o csv     # spreadsheet friendly
lemmy <command> -o tsv     # tab-separated
lemmy <command> -o url     # just the URL column
lemmy <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
lemmy <command> --fields id,title,url
```
...
```bash
lemmy <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
lemmy <command>            # a table, because this is a terminal
lemmy <command> | wc -l    # JSONL, because this is a pipe
```
